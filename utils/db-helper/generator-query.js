...
function queryPage({ filters = [], pagination = {}, sorter = {}, groupBy = [] }) {
  const query = { filter: '', having: '', pagination: '', sort: '', group: '' }
  ...
  for (const key in filters) {
    if (Array.isArray(filters[key])) {
      ...
      query.filter += `AND LOWER(${key}) LIKE LOWER(CONCAT('%', ${mysql.escape(keyword)}, '%')) `
    }
  }
  ...
  if (Array.isArray(groupBy) && groupBy.length) {
    query.group = `GROUP BY ${groupBy.join(", ")} `
  }
